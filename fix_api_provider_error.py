...
class APIProviderDiagnostic:
    ...
    def check_environment_variables(self) -> Dict:
        ...
        for var, description in required_vars.items():
            value = os.getenv(var)
            status = "[OK] Found" if value else "[FAIL] Missing"
            env_status[var] = {
                'status': status,
                'description': description,
                'value': '***' if var.endswith('TOKEN') or var == 'MONGO_URI' else value
            }
    ...
    def check_huggingface_api_availability(self) -> Dict:
        ...
        print("[SEARCH] Testing Hugging Face API Connectivity...")
        
        api_status = {
            'connection': '[FAIL] Failed',
            'model_access': '[FAIL] Failed',
            'error_message': None
        }
        ...
        try:
            ...
            if response.status_code == 200:
                api_status['connection'] = '[OK] Connected'
            else:
            ...
            try:
                ...
                if test_response.status_code == 200:
                    api_status['model_access'] = '[OK] Model Access OK'
                else:
    ...
    def check_model_availability(self, model_names: List[str]) -> Dict:
        ...
        print("[SEARCH] Checking Model Availability...")
    ...
    def generate_fix_script(self) -> str:
        ...
        print("[TOOLS] Generating Fix Script...")
        
        fix_script = """#!/bin/bash
# Auto-generated fix script for API Provider Error

echo "[START] Applying fixes for API Provider Error..."
...
"""
        ...
        if env_vars.get('HF_API_TOKEN', {}).get('status') == '[FAIL] Missing':
...
"""
        ...
        if env_vars.get('USE_AI_FEATURES', {}).get('status') == '[FAIL] Missing':
...
"""
    ...
    def create_fallback_code(self) -> str:
        ...
        print("🛡️  Creating Fallback Code...")
...
def safe_get_recommendations(learner_id, api_base_url="http://localhost:5000"):
    ...
    try:
        ...
        print("🛡️  Using local fallback recommendations...")
...
"""
        ...
        if api_status.get('connection') == '[FAIL] Failed':
            report += "- [WEB] **High Priority**: Check internet connectivity and API access\n"
        
        report += "- 🛡️ **Recommended**: Implement fallback recommendation system\n"
        report += "- [UPDATE] **Recommended**: Add proper error handling for external APIs\n"
        report += "- [NOTE] **Recommended**: Use environment variables for configuration\n"
        
        return report

def main():
    """Main diagnostic function"""
    print("[START] Starting API Provider Error Diagnostic...")
    print("=" * 50)
    ...
    print("\n" + "=" * 50)
    print("[STATS] Diagnostic Complete!")
    print("\nFiles generated:")
    print("- [WORK]  fix_api_error.sh (executable fix script)")
    print("- 🛡️  fallback_recommendations.py (fallback code)")
    print("- [DOC] api_diagnostic_report.md (detailed report)")
